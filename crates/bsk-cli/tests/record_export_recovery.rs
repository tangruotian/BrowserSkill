    ...
    let request = RequestFrame {
        ...
        params: Some(
            serde_json::to_value(HandshakeParams {
                ...
                label: "Record export recovery test".into(),
            })
            .unwrap(),
        ),
    };
...
fn sample_trace() -> TraceV3 {
    TraceV3 {
        version: 3,
        purpose: Some("export recovery".into()),
        ...
        while let Some(Ok(Message::Text(text))) = ws.next().await {
            ...
            let body = match request.method {
                ...
                Method::ToolRecordStop => {
                    record_stop_calls.fetch_add(1, Ordering::SeqCst);
                    ResponseBody::Ok(
                ...
                _ => ResponseBody::Err(bsk_protocol::RpcError {
...
fn write_record_session(home: &Path, session_id: &str) {
    std::fs::create_dir_all(home).unwrap();
    std::fs::write(
        home.join("record-session.json"),
        format!("{{\"session_id\":\"{session_id}\",\"started_at\":\"1\"}}\n"),
    )
    .unwrap();
}
...
async fn existing_trace_json_file_is_rejected_before_record_stop() {
    ...
    write_record_session(&home, &session_id);

    let first = run_record_stop(&home, &output).await;
    let stderr = String::from_utf8_lossy(&first.stderr);
    assert!(
        !first.status.success(),
        "legacy JSON --output should fail: {stderr}"
    );
    assert!(
        stderr.contains("JSON file") || stderr.contains("bundle directory"),
        "{stderr}"
    );
    let state: serde_json::Value =
        serde_json::from_str(&std::fs::read_to_string(home.join("record-session.json")).unwrap())
            .unwrap();
    assert_eq!(state["session_id"], session_id);
    assert_eq!(
        record_stop_calls.load(Ordering::SeqCst),
        0,
        "existing trace.json must be rejected before the extension returns the Trace"
    );
    assert_eq!(session_stop_calls.load(Ordering::SeqCst), 0);
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn failed_bundle_export_can_be_recovered() {
    let temp = tempfile::Builder::new()
    ...
    assert!(
        !first.status.success(),
        "blocked states path should fail export: {stderr}"
    );
    assert!(
        home.join("record-recovery.json").exists(),
        "completed Trace must be saved before the failed export"
    );
    assert!(
        !home.join("record-session.json").exists(),
        "session state is cleared after the extension returns the Trace"
    );
    assert_eq!(record_stop_calls.load(Ordering::SeqCst), 1);
    assert_eq!(session_stop_calls.load(Ordering::SeqCst), 1);

    std::fs::remove_file(output.join("states")).unwrap();
    ...
    assert_eq!(
        record_stop_calls.load(Ordering::SeqCst),
        1,
        "recovery must not call the extension again"
    );
